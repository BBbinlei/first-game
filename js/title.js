...
(function () {
  ...
  function create() {
    ...
    const doorMat = new THREE.MeshStandardMaterial({
      ...
      emissive: 0x6b4a10, emissiveIntensity: 0.5,
    });
    ...
    const rim = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xffd873, emissive: 0xffaa33, emissiveIntensity: 1.1, metalness: 0.6, roughness: 0.3 })
    );
    ...
    const dialMat = new THREE.MeshStandardMaterial({ color: 0x2a2a2a, metalness: 0.6, roughness: 0.4, emissive: 0x1a1a1a });
    ...
    const silMat = new THREE.MeshStandardMaterial({ color: 0x120e1a, roughness: 1 });
    const towers = [];
    for (let i = 0; i < 8; i++) {
  ...
  function buildSilhouetteRaccoon() {
    ...
    const mat = new THREE.MeshStandardMaterial({ color: 0x0c0a12, roughness: 1 });

    const body = new THREE.Mesh(new THREE.SphereGeometry(0.34, 12, 12), mat);
    ...
    [-0.13, 0.13].forEach((x) => {
      const ear = new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.12, 8), mat);
    ...
    group.add(tail);
    for (let i = 0; i < 5; i++) {
      const seg = new THREE.Mesh(new THREE.SphereGeometry(0.15 - i * 0.015, 8, 8), mat);
